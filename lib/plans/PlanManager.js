...
class PlanManager {
  ...
  async fetchBulk ({ plan_ids = [], product_id, page_count = 1, all = false } = {}) {
    ...
    for (let page = 1; page <= page_count; page++) {
      ...
      if (fetchResponse.statusCode !== 200) throw fetchResponse.json;
  ...
  async create (template) {
    if (!(template instanceof PlanTemplate)) throw new Error("Can only create plans from a plan template.");

    const templatePayload =template.toJSON();
    const createResponse = await this.client.request("POST", "/v1/billing/plans", {}, { "Prefer": "return=representation" }, templatePayload);

    if (createResponse.statusCode === 201) {
      const instance = new Plan(this.client, createResponse.json);
      if (this.client.keep_cache) this.cache.set(instance.id, instance);
      return instance;
    } else {
      throw createResponse.json || false;
    }
  }
}
